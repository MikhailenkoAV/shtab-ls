...
test("GitHub Pages export contains the main application sections", async () => {
  ...
  assert.match(source, /label="Главная"/);
  assert.match(source, /label="Документация"/);
  assert.match(source, /label="Настройки"/);
  ...
  assert.match(source, /Пермь/);
  assert.match(source, /Магадан/);
