...
export function FlightBookView({
  ...
  return <div className="flight-book-layout">
    <section className="panel flight-book-summary">
      ...
      <div className="table-scroll"><table className="flight-book-table"><thead><tr><th>Тип ВС</th><th>Общий</th><th>КВС</th><th>2П</th><th>Инструктор</th><th>Ночь</th><th>ППП</th><th>Заходы ППП</th><th>Из журнала</th></tr></thead><tbody>
      ...
      <p className="flight-book-caveat">КВС, инструкторский и ночной налёт пополняются автоматически из журнала. Значения 2П, ППП и заходов ППП пока добавляются через исходную контрольную точку.</p>
    </section>

    <section className="flight-book-bottom-grid">
      <article className="panel baseline-history">
        <div className="panel-heading"><div><p className="eyebrow">Исходные данные</p><h2>Контрольные точки</h2></div></div>
        {!personBaselines.length ? <div className="panel-empty">Исходный налёт ещё не внесён. Сумма строится только по журналу сайта.</div> : <div className="baseline-items">{personBaselines.map((baseline, index) => <div key={baseline.id} className={index === 0 ? "active" : ""}><div><strong>{displayDate(baseline.date)}</strong><span>{baseline.source || "Источник не указан"} · {baseline.rows.length} типов ВС</span>{baseline.note && <small>{baseline.note}</small>}</div><div>{index === 0 && <i>Используется</i>}<button onClick={() => setEditing(baseline)}>Изменить</button><button className="delete" onClick={() => {
...
function BaselineModal({
  ...
  const [rows, setRows] = useState<DraftRow[]>(baseline?.rows.length
    ...
    : (person.aircraftTypes.length ? person.aircraftTypes : [""]).map(emptyDraftRow));
  ...
  const durationFields: Array<{ key: "total" | "pic" | "secondPilot" | "instructor" | "night" | "ifr"; label: string }> = [
    ...
    { key: "pic", label: "КВС" },
    { key: "secondPilot", label: "2П" },
    { key: "instructor", label: "Инструктор" },
    { key: "night", label: "Ночь" },
    { key: "ifr", label: "ППП" },
  ];
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal extra-wide" role="dialog" aria-modal="true"><header><div><p className="eyebrow">Лётная книжка · {person.name}</p><h2>{baseline ? "Изменить исходный налёт" : "Новая контрольная точка"}</h2><span>Это накопленный налёт на выбранную дату, а не отдельная запись о полёте</span></div><button className="modal-close" aria-label="Закрыть" onClick={onClose}>×</button></header><form className="form-stack" onSubmit={(event) => {
    ...
    <label className="field"><span>Общее примечание</span><input value={note} onChange={(event) => setNote(event.target.value)} /></label>
    <div className="baseline-row-list">{rows.map((row) => <article key={row.id} className="baseline-row-card">
      ...
      <label className="field"><span>Примечание по типу</span><input value={row.note} onChange={(event) => updateRow(row.id, { note: event.target.value })} /></label>
    </article>)}</div>
    <datalist id="flight-book-aircraft-types">{person.aircraftTypes.map((aircraftType) => <option key={aircraftType} value={aircraftType} />)}</datalist>
    <button type="button" className="secondary-button add-baseline-type" onClick={() => setRows((current) => [...current, emptyDraftRow()])}>+ Добавить тип ВС</button>
