  ...
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="modal wide" role="dialog" aria-modal="true">
      ...
      <form className="form-stack" onSubmit={(event) => {
        ...
        <div className="form-grid two certification-date-grid"><label className="field"><span>Дата выдачи</span><input type="date" value={form.issuedDate} onChange={(event) => updateWithCalculatedEnd({ issuedDate: event.target.value })} /></label><label className="field"><span>Дата окончания действия</span><input type="date" value={form.endDate} onChange={(event) => update("endDate", event.target.value)} /></label></div>
        {documentValidityLabel(selectedDefinition, form.operator) && <small className="document-validity-hint">Дата окончания рассчитана автоматически: {documentValidityLabel(selectedDefinition, form.operator)}</small>}
