...
export type ExpiryState = { level: "expired" | "alert14" | "alert30" | "valid" | "undated" | "incomplete"; label: string; days: number | null };
...
export function getExpiryState(record: CertificationRecord, today = new Date()): ExpiryState {
  if (!record.endDate) {
  ...
  if (days <= 30) return { level: "alert30", label: `Осталось ${days} дн.`, days };
...
export function PersonalFilesView({ people, records, onImportClick, onUpsert, onDelete }: { people: PersonRef[]; records: CertificationRecord[]; onImportClick: () => void; onUpsert: (record: CertificationRecord) => void; onDelete: (id: string) => void }) {
  ...
  const personRecords = useMemo(() => records.filter((record) => record.personId === personId).sort((a, b) => {
    const order = { expired: 0, alert14: 1, alert30: 2, incomplete: 3, valid: 4, undated: 5 };
    return order[getExpiryState(a).level] - order[getExpiryState(b).level] || (a.endDate || "9999").localeCompare(b.endDate || "9999");
  }), [records, personId]);
  const counts = personRecords.reduce((result, record) => { result[getExpiryState(record).level] += 1; return result; }, { expired: 0, alert14: 0, alert30: 0, valid: 0, undated: 0, incomplete: 0 });
  const visible = personRecords.filter((record) => { const state = getExpiryState(record); const matchesStatus = !attentionOnly || ["expired", "alert14", "alert30", "incomplete"].includes(state.level); return matchesStatus && `${record.category} ${record.certificationType} ${record.aircraftType} ${record.organization} ${record.number}`.toLocaleLowerCase("ru-RU").includes(query.trim().toLocaleLowerCase("ru-RU")); });
...
export function ImportAviabitModal({ people, onClose, onSubmit }: { people: PersonRef[]; onClose: () => void; onSubmit: (payload: ImportPayload) => void }) {
  ...
  const summary = parsed?.records.reduce((result, record) => { result[getExpiryState(record).level] += 1; return result; }, { expired: 0, alert14: 0, alert30: 0, valid: 0, undated: 0, incomplete: 0 });
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><section className="modal wide" role="dialog" aria-modal="true"><header><div><p className="eyebrow">Личное дело</p><h2>Импорт из Авиабит</h2><span>Поддерживается выгрузка «Сертификации» в Excel</span></div><button className="modal-close" aria-label="Закрыть" onClick={onClose}>×</button></header><form className="form-stack" onSubmit={(event) => { event.preventDefault(); if (parsed && name.trim()) onSubmit({ targetPersonId: target === "__new__" ? null : target, personName: name.trim(), records: parsed.records }); }}>
    ...
    {parsed && <><div className="import-summary"><div><strong>{parsed.records.length}</strong><span>строк найдено</span></div><div><strong>{summary?.expired ?? 0}</strong><span>просрочено</span></div><div><strong>{(summary?.alert14 ?? 0) + (summary?.alert30 ?? 0)}</strong><span>истекает за 30 дней</span></div><div><strong>{summary?.incomplete ?? 0}</strong><span>нет данных</span></div></div><div className="form-grid two"><label className="field"><span>Карточка сотрудника</span><select value={target} onChange={(event) => setTarget(event.target.value)}><option value="__new__">Создать новую карточку</option>{people.map((person) => <option key={person.id} value={person.id}>{person.name}</option>)}</select></label><label className="field"><span>Ф. И. О.</span><input required disabled={target !== "__new__"} value={target === "__new__" ? name : people.find((person) => person.id === target)?.name ?? ""} onChange={(event) => setName(event.target.value)} /></label></div><div className="import-note"><strong>При повторной загрузке</strong><span>Записи Авиабит выбранного сотрудника обновятся; ручные записи сохранятся.</span></div></>}
