...
export function TrainingMatrixView({ people, records, definitions, busyEntries, onOpenPerson, onPlan, onNotify }: {
  ...
  const rows = people.filter((person) => person.active)
    .filter((person) => !operator || person.qualifications.some((item) => item.operators.includes(operator)))
    .filter((person) => !aircraft || person.aircraftTypes.includes(aircraft))
    .map((person) => {
      ...
      const cells = columns.map((definition) => {
        const matches = current.filter((record) => normalized(`${record.certificationType} ${record.documentType}`).includes(normalized(definition.name)) && (!aircraft || !record.aircraftType || record.aircraftType.replace(/\s+/g, "") === aircraft.replace(/\s+/g, "")));
        ...
        return { definition, record, state };
      });
      return { person, cells };
  ...
  return <section className="panel training-matrix-panel">
    ...
    <div className="training-matrix-scroll"><table className="training-matrix"><thead><tr><th>Сотрудник</th>{columns.map((item) => <th key={item.id}>{item.name}</th>)}</tr></thead><tbody>{rows.map(({ person, cells }) => <tr key={person.id}><th><button onClick={() => onOpenPerson(person.id)}>{person.name}</button><small>{person.aircraftTypes.join(", ")}</small></th>{cells.map(({ definition, record, state }) => {
      ...
      return <td className={state.level} key={definition.id}><strong>{state.label}</strong><span>{record ? displayDate(record.endDate) : "Документ не внесён"}</span>{["expired", "alert14", "alert45"].includes(state.level) && <button disabled={planned} onClick={() => { onPlan({ id: uid(), personId: person.id, activity: "periodic_training", dateFrom: date, dateTo: date, note: definition.name }); onNotify(planned ? "Подготовка уже запланирована" : `Подготовка предложена на ${displayDate(date)}`); }}>{planned ? "В плане" : `В план: ${displayDate(date)}`}</button>}</td>;
    })}</tr>)}</tbody></table></div>
  </section>;
}
